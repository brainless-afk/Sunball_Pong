...
# ToDO acceleration, decceleration paddle
# change process to _input?
...
func _on_paddle_area_enter( area ):
	...
	if (is_network_master()):
		var random_dir = rand_range(-0.3, 0.3)
		
		# get impact point of ball on paddle
		var posDiff = position - area.position
		posDiff = posDiff.normalized()
		#var posImpact = abs(posDiff.y)
		
		area.rpc("bounce",left, random_dir, posDiff) #random for new direction generated on each peer
